...
pub enum InstructionMode {
    ...
    #[default]
    Replace,
    /// Preserve the harness default and append this instruction after it.
    Append,
}
...
impl AdapterConfigSupport {
    fn supports_system_instruction_mode(&self, mode: InstructionMode) -> bool {
        if !self
            .accepts
            .contains(&AdapterConfigField::SystemInstructions)
        {
            return false;
        }
        self.system_instruction_modes
            .as_ref()
            .map_or(mode == InstructionMode::Replace, |modes| {
                modes.contains(&mode)
            })
    }

    fn supported_system_instruction_modes(&self) -> Vec<InstructionMode> {
        if !self
            .accepts
            .contains(&AdapterConfigField::SystemInstructions)
        {
        ...
        self.system_instruction_modes
            .clone()
            .unwrap_or_else(|| vec![InstructionMode::Replace])
    }
}
...
pub(crate) fn adapter_config_compatibility_issues(
    ...
    if let Some(system) = config
        ...
        .and_then(|instructions| instructions.system.as_ref())
    {
        if !accepts(AdapterConfigField::SystemInstructions) {
            ...
            .config
            .supports_system_instruction_mode(system.mode)
        {
            let supported = descriptor
                .config
                .supported_system_instruction_modes()
                .into_iter()
                .map(InstructionMode::as_str)
                .collect::<Vec<_>>()
                .join(", ");
            issues.push(incompatible(
                "instructions.system.mode".to_string(),
                format!(
                    "mode `{}` is not supported; supported modes: {supported}",
                    system.mode.as_str()
                ),
            ));
        }
...
fn validate_adapter_descriptor_shape(descriptor: &AdapterDescriptor, path: &Path) -> Result<()> {
    ...
    if let Some(modes) = &descriptor.config.system_instruction_modes {
        if !descriptor
            .config
            .accepts
            .contains(&AdapterConfigField::SystemInstructions)
        {
            return invalid_adapter_descriptor(
                path,
                "config.system_instruction_modes requires config.accepts to include `instructions.system`",
            );
        }
        if modes.is_empty() {
            return invalid_adapter_descriptor(
                path,
                "config.system_instruction_modes must contain at least one mode",
            );
        }
        if modes
            .iter()
            ...
            .any(|(index, mode)| modes[..index].contains(mode))
        {
            return invalid_adapter_descriptor(
                path,
                "config.system_instruction_modes must not contain duplicate modes",
            );
        }
    }
    for (field, schema) in [
...
mod tests {
    ...
    #[test]
    fn append_system_instruction_survives_planning_for_supported_adapter() {
        let mut config = typed_config("nvidia.fabric.claude");
        config.instructions = Some(InstructionsConfig {
            system: Some(InstructionConfig {
                content: "Follow the repository review policy.".to_string(),
        ...
        let plan = resolve_run_plan_from_config(
            config,
            ResolveContext::new("/tmp/fabric-append-instruction"),
        )
        .expect("Claude supports append instructions");
        ...
        assert_eq!(
            plan.config
                ...
                .as_ref()
                .and_then(|instructions| instructions.system.as_ref())
                .map(|instruction| instruction.mode),
            Some(InstructionMode::Append)
        );
        assert_eq!(
            plan.agent_config
                .instructions
    ...
    #[test]
    fn unsupported_system_instruction_mode_fails_planning_at_exact_field() {
        ...
        let error = resolve_run_plan_from_config(
            config,
            ResolveContext::new("/tmp/fabric-unsupported-instruction-mode"),
        )
        .expect_err("Codex advertises replace-only instructions");
    ...
    fn legacy_descriptor_instruction_claim_is_replace_only() {
        ...
        config
            .instructions
            .as_mut()
            .and_then(|instructions| instructions.system.as_mut())
            .expect("system instruction")
            .mode = InstructionMode::Append;
        let issues = adapter_config_compatibility_issues(&config, Some(&descriptor));

        assert_eq!(issues.len(), 1);
        assert_eq!(issues[0].field, "instructions.system.mode");
        assert!(issues[0].reason.contains("supported modes: replace"));
    }
    ...
    fn rejects_invalid_system_instruction_mode_claims() {
        ...
        without_acceptance
            .config
            .accepts
            .retain(|field| *field != AdapterConfigField::SystemInstructions);
        let error = validate_adapter_descriptor_shape(&without_acceptance, &path)
            .expect_err("mode claims require system-instruction acceptance");
        assert!(matches!(
            error,
            FabricError::InvalidAdapterDescriptor { message, .. }
                if message.contains("requires config.accepts")
        ));

        let mut empty = descriptor.clone();
        empty.config.system_instruction_modes = Some(Vec::new());
        let error = validate_adapter_descriptor_shape(&empty, &path)
            .expect_err("mode claims must not be empty");
        assert!(matches!(
            error,
            FabricError::InvalidAdapterDescriptor { message, .. }
        ...
        duplicate.config.system_instruction_modes =
            Some(vec![InstructionMode::Replace, InstructionMode::Replace]);
        let error = validate_adapter_descriptor_shape(&duplicate, &path)
            .expect_err("mode claims must be unique");
        assert!(matches!(
            error,
            FabricError::InvalidAdapterDescriptor { message, .. }
                if message.contains("duplicate modes")
        ));
    }
