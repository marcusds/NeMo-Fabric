...
Python-backed Relay integrations require `nemo-relay>=0.9,<0.10`. NeMo Fabric
does not merge the user `plugins.toml` with its runtime-owned configuration.
Remove or relocate the user `plugins.toml` before starting a Relay-enabled Deep
Agents, Hermes Agent, or mini-SWE-agent runtime. Relay 0.8 dropped
repository-local `.nemo-relay/plugins.toml`. System
configuration in `/etc/nemo-relay/plugins.toml` remains administrator
controlled.

#### Migrate Relay Observability Configurations from 0.6

Relay 0.7 and later require observability schema version 3. Move flat OpenTelemetry
...
Claude, Codex, and Pi harness extras install their compatible CLI binary without
the Relay Python package. Install the `>=0.9,<0.10` range:

```bash
pip install "nemo-relay-cli-bin>=0.9,<0.10"
```

If `cargo-binstall` is available, install the pinned CLI with Cargo:

```bash
cargo binstall nemo-relay-cli --version 0.9.0
```

After installation, run `nemo-relay --version` and confirm the version is in
the `>=0.9,<0.10` range. When Relay is enabled with
