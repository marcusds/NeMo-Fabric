...
`instructions.system` with `mode="replace"`; the adapter passes that normalized
value to the Codex SDK as `base_instructions`. The adapter rejects `append`.
...
NeMo Fabric supports the external NeMo Relay CLI in the `>=0.9,<0.10` range.
